    ...
    for text_loader_name, text_loader in text_loaders.items():
        ...
        with torch.no_grad():
            for idx, batch in enumerate(text_loader):
                word_embeddings, pos_one_hots, _, sent_lens, motions, _, m_lens, _ = batch[:8]
...
def evaluate_bleu_rouge_cider(text_loaders, file):
    ...
    for text_loader_name, text_loader in text_loaders.items():
        ...
        with open('gt.json', 'w') as f:
            json.dump(text_loader.dataset.all_caption_list, f, indent=4)
        with open('pred.json', 'w') as f:
            json.dump(text_loader.dataset.generated_texts_list, f ,indent=4)
